//
//  Navigator.Compact.Animations.YM.swift
//  Calendar
//
...
extension CalendarVC.Navigator.Compact.Animations.YM: UIViewControllerAnimatedTransitioning {
    ...
    final func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        // контроллеры и вьюхи
        guard
            let calendarVC = transitionContext.viewController(forKey: .from) as? CalendarVC,
        ...
        // поставим вьюху контроллера месяца в нативное положение
        ...
        CalendarVC.Navigator.Compact.Animations.execute(
            ...
            from:     calendarVC.layout(with: monthVC),
            to:       monthVC.layout(with: yearVC)
        )
    }
}
